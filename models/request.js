...
module.exports = class Request { 
  constructor(sir_name,first_name,email,phone_number,mineralName,price,amount,country,date,status,description,req_type,license_number,mineral_type,weight){
  ...
  save() {
    ...
    try{
      db.execute('INSERT INTO requests (sir_name, first_name, email, phone_number, mineral_name, price, amount, country, date, status, description, req_type, response,license_number, mineral_type, weight) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [this.sir_name, this.first_name, this.email, this.phone_number, this.mineralName, this.price, this.amount, this.country, this.date, this.status, this.description, this.req_type, '', this.license_number, this.mineral_type, this.weight]);
    }catch(err){
